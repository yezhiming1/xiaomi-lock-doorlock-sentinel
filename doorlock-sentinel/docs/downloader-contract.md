...
目标下载器版本为 `V0.0.8`。下载器应关闭自身录像删除策略，由 115 备份专项任务在远端校验成功后统一决定删除。经过/停留事件触发后等待 120 秒执行增量下载，每日 03:30 只作为遗漏核对。
...
```text
xiaomi_lock_20260829T184213.mp4
```

事件时间按以下顺序确定：

1. 合法旁车文件中的带时区时间。
2. 下载器文件名中的北京时间；同秒冲突允许 `-02`、`-03` 序号。
3. 文件修改时间兜底。

管理页面同时保存 `downloaded_at` 和 `time_source`，不会把识别程序扫描时间冒充门锁事件时间。文件内容 SHA-256 是去重依据，文件名不是唯一键。

派生图片使用录像名主干：人脸裁剪图追加 `-a001.jpg`，带人脸框场景图追加
`-b001.jpg`；多人轨迹按 `002`、`003` 递增。历史改名时必须先停止识别容器，先由
下载器迁移 MP4 与 HA Store，再由 V0.0.4 迁移数据库和图片。两个阶段都必须先 dry-run，
且不修改任何 115 对象或回执。
