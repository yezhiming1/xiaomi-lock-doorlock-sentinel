...
def _existing_result(session: Session, idempotency_key: str) -> dict[str, Any] | None:
    operation = session.scalar(
        select(ManualOperation).where(
            ManualOperation.idempotency_key == idempotency_key
        )
    )
    ...
    times = [
        ...
        for value in session.scalars(
            select(Event.occurred_at).where(
                Event.id.in_([item.event_id for item in observations])
            )
        )
    ]
...
def merge_people(
    ...
    *,
    ...
    source_tracks = list(
        session.scalars(select(FaceTrack).where(FaceTrack.person_id == source.id))
    )
    target_track_ids = set(
        session.scalars(
            select(FaceTrack.id).where(FaceTrack.person_id == target.id)
        )
    )
    if _has_cannot_link(
        session,
    ...
    prototypes = list(
        session.scalars(
            select(FacePrototype).where(FacePrototype.person_id == source.id)
        )
    )
    ...
    target_observation_events = set(
        session.scalars(
            select(PersonObservation.event_id).where(
                PersonObservation.person_id == target.id
            )
        )
    )
    ...
    for observation in list(
        session.scalars(
            select(PersonObservation).where(
                PersonObservation.person_id == source.id
            )
        )
    ):
...
def merge_clusters(
    ...
    *,
    ...
    target_track_ids = {
        track.id for _member, track in _cluster_members(session, target.id)
    }
    if _has_cannot_link(
        session,
...
def split_cluster(
    ...
    *,
    ...
    new_cluster = UnknownCluster(
        ...
        centroid=pack_vector(
            unpack_vector(first_track.embedding, first_track.embedding_dimension)
        ),
...
def undo_operation(
    ...
    *,
    ...
    if original.operation == "rename_person":
        ...
        tracks = list(
            session.scalars(select(FaceTrack).where(FaceTrack.id.in_(track_ids)))
        )
        ...
        session.execute(
            delete(PersonObservation).where(PersonObservation.person_id == person.id)
        )
        session.execute(
            delete(FacePrototype).where(FacePrototype.person_id == person.id)
        )
        ...
        for prototype in session.scalars(
            select(FacePrototype).where(
                FacePrototype.id.in_(before["prototype_ids"])
            )
        ):
        ...
        for member in session.scalars(
            select(UnknownClusterMember).where(
                UnknownClusterMember.id.in_(member_ids)
            )
        ):
        ...
        for member in session.scalars(
            select(UnknownClusterMember).where(
                UnknownClusterMember.track_id.in_(track_ids)
            )
        ):
            member.cluster_id = source.id
        for track in session.scalars(
            select(FaceTrack).where(FaceTrack.id.in_(track_ids))
        ):
            track.unknown_cluster_id = source.id
        session.flush()
